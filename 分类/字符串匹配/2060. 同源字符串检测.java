//diff means have diff characters can be ignored 
//出现数字就说明另一个字符串可以忽略n个字符
...
class Solution {
    ...
    private boolean dfs(int i, int j, int diff, char[] s1, char[] s2, Boolean[][][] dp) {
        ...
        if (i < s1.length && j < s2.length && diff == 0 && s1[i] == s2[j]) {             //只有diff==0时才能同时消除字符 
        ...
        if (i < s1.length && !isDigit(s1[i]) && diff > 0 && dfs(i + 1, j, diff - 1, s1, s2, dp)) {        //可以消除一个s1的字符
        ...
        for (int k = i, val = 0; k < s1.length && isDigit(s1[k]); ++k) {        //出现数组 diff增加 后面要在另一个字符串中删掉
            val = val * 10 + (s1[k] - '0');
